...
- **PHP 8.1+** (bez frameworku) — czysty, typowany kod: `readonly` value
  objects, `enum`, ścisłe typy.
- **Composer** wyłącznie do autoloadingu PSR-4 (`App\` → `src/`) — zero
  zależności runtime.
- Prosty, własny router + warstwa widoków oparta o zwykłe pliki `.php`
  (bez silnika szablonów) — mniej ruchomych części, łatwe do rozwijania.
...
public/                 punkt wejścia HTTP (front controller) + assets
  index.php             routing + kompozycja obiektów (tu podłącza się CRM)
...
src/
  ...
  Http/Controller/        DashboardController, DealsController,
                          DealController, ActivitiesController,
                          TasksController
  Support/                Router, Request, View, DateHelper, TimeFormatter,
                          helpers.php (e(), money(), qs())

templates/                widoki (czysty PHP, bez logiki biznesowej)
...
- **Service/Crm** — jedyne miejsce, które "wie", skąd biorą się dane.
  ...
  Reszta aplikacji zna tylko ten interfejs.
- **Service (bez Crm)** — logika biznesowa: liczenie dni, statystyk,
  wykrywanie flag, filtrowanie. Nie wie nic o HTTP ani o źródle danych.
- **Http/Controller** — łączy CRM + logikę biznesową + widok. Nie zawiera
  logiki liczenia niczego.
...
dołączony `public/.htaccess` przekierowuje wszystkie żądania do
`index.php` (wymaga `mod_rewrite`). Na nginx wystarczy standardowy
`try_files $uri /index.php?$query_string;` z document rootem w `public/`.
...
2. Zmapuj odpowiedzi API Bitrix24 (`crm.deal.list`, `crm.activity.list`,
   ...
   `User`).
3. W `public/index.php` podmień jedną linię:
   ```php
   $crm = new MockCrmService($now);
   // →
   $crm = new Bitrix24CrmService($webhookUrl, $now);
   ```
